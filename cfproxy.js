...
let paramName = 'xyz';  // 参数名称，可改为任意值如 proxy、server、addr 等
    ...
    async fetch(请求,env) {
        try {
            ...
            if (pathname.startsWith(paramPrefix)) {
                try {
                    pathProxyIP = decodeURIComponent(pathname.substring(paramPrefix.length)).trim();
                } catch (e) {
                    // 忽略错误
                }
            ...
            if (请求.headers.get('Upgrade') === 'websocket') {
                ...
                try {
                    wsPathProxyIP = decodeURIComponent(pathname.substring(paramPrefix.length)).trim();
                } catch (e) {
                    // 忽略错误
                }
